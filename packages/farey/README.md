...
The [Farey sequence](https://en.wikipedia.org/wiki/Farey_sequence) F_n is the sequence of completely reduced fractions between 0 and 1, with denominators ≤ n, in ascending order.

A **Farey range** applies a Farey sequence to subdivide an arbitrary interval into rational points. In other words, it is the (sorted) union of all the rational ranges with (reciprocal of their) steps equal to the elements of the Farey sequence. 
