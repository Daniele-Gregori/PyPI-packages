...
from spreadsheet_toolkit import (
    import_all,
    import_cells,
    import_formulas,
    import_sheets,
...
class TestBackwardCompatibleAliases:
    def test_spreadsheet_index_to_position_is_index_to_position(self):
        assert spreadsheet_index_to_position is index_to_position
        assert spreadsheet_index_to_position("AA1") == (1, 27)

    def test_position_to_spreadsheet_index_is_position_to_index(self):
        assert position_to_spreadsheet_index is position_to_index
        assert position_to_spreadsheet_index((1, 27)) == "AA1"


class TestImportSheets:
    """Ports ``TestImportSheets`` (0.1.x); equivalent to ``import_all(...)[0]``."""
...
class TestImportCells:
    """Ports ``TestImportCells`` (0.1.x). Consistent with ``import_all``:
    empty cells and uncached formula values are ``""`` (were ``None``)."""
    ...
    def test_formula_cells_return_cached_value_or_empty(self, sample_xlsx):
        data = import_cells(sample_xlsx)
        assert data[0][2] in ("", 30)
    ...
    def test_empty_sheet(self, empty_xlsx):
        assert import_cells(empty_xlsx) == [[""]]  # was [[None]] in 0.1.x

    def test_accepts_string_path(self, sample_xlsx):
        assert import_cells(str(sample_xlsx))[0][0] == 10

    def test_accepts_path_object(self, sample_xlsx):
        assert import_cells(Path(sample_xlsx))[0][0] == 10

    def test_result_is_a_copy_of_the_cache(self, sample_xlsx):
        import_cells(sample_xlsx)[0][0] = "mutated"
        assert import_cells(sample_xlsx)[0][0] == 10


class TestImportFormulas:
    """Ports ``TestImportFormulas`` (0.1.x). Consistent with ``import_all``:
    no leading ``=`` (was kept) and ``""`` for non-formula cells (were
    their values)."""
    ...
    def test_empty_sheet(self, empty_xlsx):
        assert import_formulas(empty_xlsx) == [[""]]
